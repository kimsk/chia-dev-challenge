...
def get_genesis_ticket(number, block_height) -> Program:
    _, oracle_puzzhash = get_oracle(block_height)
...
def get_ticket(number, block_height) -> Optional[Tuple[Program, bytes32]]:
    if (number not in VALID_NUMBER_RANGES):
        return None
    ticket: Program = get_genesis_ticket(number, block_height).curry(
        block_height
    )
    return ticket, ticket.get_tree_hash()

def generate_tickets(block_height) -> List[Tuple[Program, bytes32]]:
    return [
        get_ticket(number, block_height)
